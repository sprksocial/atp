...
export const verifyDiff = async (
  ...
  const updateStorage = repo
    ...
    : stagedStorage;
  const updated = verifyRepoRoot(
    updateStorage as ReadableBlockstore,
    updateRoot,
    did,
    signingKey,
  );
...
const verifyRepoRoot = (
  ...
  const repo = ReadableRepo.load(storage, head);
  if (did !== undefined && repo.did !== did) {
    throw new RepoVerificationError(`Invalid repo did: ${repo.did}`);
  }
  if (signingKey !== undefined) {
    const validSig = util.verifyCommitSig(repo.commit, signingKey);
    if (!validSig) {
...
export const verifyProofs = async (
  ...
  const commit = blockstore.readObj(car.root, def.commit);
  if (commit.did !== did) {
    throw new RepoVerificationError(`Invalid repo did: ${commit.did}`);
  }
  const validSig = util.verifyCommitSig(commit, didKey);
  if (!validSig) {
    throw new RepoVerificationError(
...
export const verifyRecords = async (
  ...
  const validSig = util.verifyCommitSig(commit, signingKey);
  if (!validSig) {
    throw new RepoVerificationError(
      `Invalid signature on commit: ${car.root.toString()}`,
    );
  ...
  for (const leaf of leaves) {
    ...
    const record = blockstore.attemptReadRecord(leaf.value);
    if (record) {
      records.push({
        collection,
        rkey,
