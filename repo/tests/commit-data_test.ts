...
import { blocksToCarFile, Repo, verifyProofs, WriteOpAction } from "../mod.ts";
import { MemoryBlockstore } from "../storage/index.ts";
import { assertRejects } from "@std/assert";
import { assertEquals } from "@std/assert/equals";

// @NOTE this test uses a fully deterministic tree structure
Deno.test("includes all relevant blocks for proof in commit data", async () => {
  ...
  // in the proof path but _is not_ in newBlocks (as it already existed in the repository)
  {
    ...
    await assertRejects(() => proofAttempt, "block not found");
  }
  ...
  for (const rkey of keys) {
    ...
    assertEquals(proofRes.unverified.length, 0);
