...
Deno.test("two deep split ", async () => {
  ...
  let proofMst = MST.load(proofStorage, await mst.getPointer());
  proofMst = await proofMst.delete(k.D2);
  const rootAfterInvert = await proofMst.getPointer();
  assertEquals(rootAfterInvert.equals(rootBeforeCommit), true);
});
...
Deno.test("two deep leafless splits ", async () => {
  ...
  const rootAfterInvert = await proofMst.getPointer();
  assert(rootAfterInvert.equals(rootBeforeCommit));
});
...
Deno.test("merge and split in multi op commit", async () => {
  ...
  const addB = (mst: MST) => mst.add(k.B2, cid);
  const addD = (mst: MST) => mst.add(k.D2, cid);
  const delC = (mst: MST) => mst.delete(k.C2);
...
Deno.test("complex multi-op commit", async () => {
  ...
  const delA = (mst: MST) => mst.delete(k.A2);
  const delG = (mst: MST) => mst.delete(k.G2);
  const addC = (mst: MST) => mst.add(k.C2, cid);
