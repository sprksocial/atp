...
// Custom varint decoding to work around std library bug
const decodeVarintCustom = (bytes: Uint8Array): number => {
  let result = 0;
  let shift = 0;
  ...
  throw new Error("Invalid varint: no terminating byte found");
};
...
export async function* writeCarStream(
  ...
  const header = new Uint8Array(cbor.encode(headerObj));
  const [ui8] = encodeVarint(header.byteLength);
  yield ui8;
  yield header;
  for await (const block of blocks) {
    const [blockUi8] = encodeVarint(
      block.cid.bytes.byteLength + block.bytes.byteLength,
    );
    yield blockUi8;
...
class BufferedReader {
  ...
  constructor(stream: Iterable<Uint8Array> | AsyncIterable<Uint8Array>) {
    this.iterator = Symbol.asyncIterator in stream
  ...
  async readVarint(): Promise<number | null> {
    ...
    const number = decodeVarintCustom(concatted);
    return number;
  }
