...
import type { RepoInputValue } from "./types.ts";
import { lexToCborValue } from "./util.ts";
...
export class BlockMap implements Iterable<[cid: Cid, bytes: Uint8Array]> {
  ...
  async add(value: RepoInputValue): Promise<Cid> {
    const bytes = encodeLexCbor(
