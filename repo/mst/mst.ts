...
export class MST {
  ...
  async attemptGetLayer(): Promise<number | null> {
    ...
    let layer = util.layerForEntries(entries);
    if (layer === null) {
  ...
  async deleteRecurse(key: string): Promise<MST> {
    ...
    if (prev?.isTree()) {
      ...
      const subTreeEntries = subtree.getEntries();
  ...
  updateEntry(index: number, entry: NodeEntry): MST {
    const update = [
      ...(this.slice(0, index)),
      entry,
      ...(this.slice(index + 1)),
  ...
  spliceIn(entry: NodeEntry, index: number): MST {
    const update = [
      ...this.slice(0, index),
      entry,
      ...this.slice(index),
  ...
  replaceWithSplit(
    ...
    const update = this.slice(0, index);
  ...
  async splitAround(key: string): Promise<[MST | null, MST | null]> {
    ...
    const leftData = this.slice(0, index);
    const rightData = this.slice(index);
    ...
    return [
      (left.getEntries()).length > 0 ? left : null,
      (right.getEntries()).length > 0 ? right : null,
    ];
  }
  ...
  async appendMerge(toMerge: MST): Promise<MST> {
    ...
    const toMergeEntries = toMerge.getEntries();
  ...
  async *carBlockStream(): AsyncIterable<CarBlock> {
    ...
    while (toFetch.size() > 0) {
      ...
      const fetched = this.storage.getBlocks(toFetch.toList());
      if (fetched.missing.length > 0) {
        throw new MissingBlocksError("mst node", fetched.missing);
      }
      for (const cid of toFetch.toList()) {
        const found = parse.getAndParseByDef(
          fetched.blocks,
          cid,
          nodeDataDef,
        );
        yield { cid, bytes: found.bytes };
        const entries = util.deserializeNodeData(this.storage, found.obj);
    ...
    const leafData = this.storage.getBlocks(leaves.toList());
  ...
  async proofForRightSib(key: string): Promise<BlockMap> {
    const index = this.findGtOrEqualLeafIndex(key);
    let found = this.atIndex(index);
    if (!found) {
      found = this.atIndex(index - 1);
    }
    let blocks: BlockMap;
    if (!found) {
