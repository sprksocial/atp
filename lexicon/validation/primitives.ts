...
import { graphemeLen, utf8Len } from "@atp/common";
import {
  type LexBoolean,
  type LexBytes,
  type LexInteger,
...
export function validate(
  ...
  switch (def.type) {
    case "boolean":
      return boolean(path, def, value);
    case "integer":
      return integer(path, def, value);
    case "string":
      return string(path, def, value);
    case "bytes":
      return bytes(path, def, value);
    case "cid-link":
      return cidLink(path, value);
    case "unknown":
      return unknown(path, value);
    default:
      return {
        success: false,
