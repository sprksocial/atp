...
// -prf
function resolveRefUris(obj: Record<string, unknown>, baseUri: string): object {
  for (const k in obj) {
    if (obj.type === "ref") {
      obj.ref = toLexUri(obj.ref as string, baseUri);
    } else if (obj.type === "union") {
      const refs = obj.refs as string[];
      obj.refs = refs.map((ref: string) => toLexUri(ref, baseUri));
    } else if (Array.isArray(obj[k])) {
      const arrayValue = obj[k] as unknown[];
      obj[k] = arrayValue.map((item: unknown) => {
        if (typeof item === "string") {
          return item.startsWith("#") ? toLexUri(item, baseUri) : item;
        } else if (item && typeof item === "object") {
          return resolveRefUris(item as Record<string, unknown>, baseUri);
        }
        return item;
      });
    } else if (obj[k] && typeof obj[k] === "object") {
      obj[k] = resolveRefUris(obj[k] as Record<string, unknown>, baseUri);
    }
  }
  return obj;
}
