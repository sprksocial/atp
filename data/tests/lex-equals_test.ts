...
import { lexEquals } from "../lex.ts";
import type { LexValue } from "../lex.ts";
import { assertThrows } from "@std/assert/throws";

function expectLexEqual(a: LexValue, b: LexValue, expected: boolean) {
  assertEquals(lexEquals(a, b), expected);
  assertEquals(lexEquals(b, a), expected);
}

Deno.test("lexEquals compares primitive values", () => {
...
Deno.test("lexEquals compares NaN and Infinity correctly", () => {
...
Deno.test("lexEquals compares arrays", () => {
...
Deno.test("lexEquals compares Uint8Arrays", () => {
...
Deno.test("lexEquals compares CIDs", () => {
...
Deno.test("lexEquals compares objects", () => {
...
Deno.test("lexEquals compares nested structures", () => {
  const lex1 = {
...
Deno.test("lexEquals allows comparing invalid numbers (floats, NaN, Infinity)", () => {
...
for (const value of [3.14, NaN, Infinity, -Infinity]) {
  Deno.test(`reference equality returns true for identical references of ${String(value)}`, () => {
...
Deno.test("lexEquals returns true for identical references", () => {
  const arr = [1, 2, 3];
...
Deno.test("throws when comparing plain object with non-allowed class instance", () => {
  // @ts-expect-error should throw
  assertThrows(() => lexEquals({}, new Map()));
  // @ts-expect-error should throw
  assertThrows(() => lexEquals(new Map(), {}));
  // @ts-expect-error should throw
  assertThrows(() => lexEquals({ foo: {} }, { foo: new Map() }));
  // @ts-expect-error should throw
  assertThrows(() => lexEquals({ foo: new Map() }, { foo: {} }));

  assertThrows(() => lexEquals({ foo: {} }, { foo: new (class {})() }));

  assertThrows(() => lexEquals({ foo: new (class {})() }, { foo: {} }));

  assertThrows(() => lexEquals({ foo: {} }, { foo: new (class Object {})() }));
});
