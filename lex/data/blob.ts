...
import { asCid, RAW_BIN_MULTICODEC, SHA2_256_MULTIHASH_CODE } from "./cid.ts";
...
export function isBlobRef(
  ...
  for (const key in input) {
    if (
      key !== "$type" && key !== "mimeType" && key !== "ref" && key !== "size"
    ) {
      return false;
    }
  }
  ...
  if (options?.strict) {
    ...
    if (cid.code !== RAW_BIN_MULTICODEC) return false;
...
export function isLegacyBlobRef(input: unknown): input is LegacyBlobRef {
  ...
  for (const key in input) {
    if (key !== "cid" && key !== "mimeType") return false;
  }
