import { CID, isPlainObject } from "@atp/data";
...
export function stringifyIssue(issue: ValidationIssue): string {
  ...
  switch (issue.code) {
    case "invalid_format":
      return `Invalid ${stringifyStringFormat(issue.format)} format${
        issue.message ? ` (${issue.message})` : ""
      }${pathStr} (got ${stringifyValue(issue.input)})`;
    case "invalid_type":
      return `Expected ${
        oneOf(issue.expected.map(stringifyExpectedType))
      } value type${pathStr} (got ${stringifyType(issue.input)})`;
    case "invalid_value":
      return `Expected ${
        oneOf(issue.values.map(stringifyValue))
      }${pathStr} (got ${stringifyValue(issue.input)})`;
    case "required_key":
  ...
  switch (typeof value) {
    ...
    case "object":
      ...
      if (isPlainObject(value)) {
        return `{${
          stringifyArray(Object.entries(value), stringifyObjectEntry)
        }}`;
      }
    // fallthrough
    default:
