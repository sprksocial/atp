...
import type { ArrayContaining } from "../core.ts";
import {
  ValidationError,
...
> = Options extends readonly [Validator<infer V>] ? V
...
export class DiscriminatedUnionSchema<
  const Discriminator extends string = string,
  const Options extends DiscriminatedUnionSchemaVariants<Discriminator> =
    DiscriminatedUnionSchemaVariants<Discriminator>,
