...
export type TypedRefSchemaValidator<
  V extends { $type?: string } = { $type?: string },
> = V extends { $type?: infer T extends string }
  ? { $type: T } & Validator<V & { $type?: T }>
  : never;

export type TypedRefGetter<V extends { $type?: string } = { $type?: string }> =
  () => TypedRefSchemaValidator<V>;

export type TypedRefSchemaOutput<
  V extends { $type?: string } = { $type?: string },
