import { z } from "zod";
import { isLastOccurrence } from "./util.ts";

export const PUBLIC_KEY_USAGE = ["verify", "encrypt", "wrapKey"] as const;
export const publicKeyUsageSchema: z.ZodEnum<{
  verify: "verify";
  encrypt: "encrypt";
  wrapKey: "wrapKey";
}> = z.enum(PUBLIC_KEY_USAGE);
 ...
 * key use.
 */
export function isSigKeyUsage(v: KeyUsage): v is "verify" {
  return v === "verify";
}
...
type PrivateKeyUsagSchema = z.ZodEnum<{
  sign: "sign";
  decrypt: "decrypt";
  unwrapKey: "unwrapKey";
  deriveKey: "deriveKey";
...
export const privateKeyUsageSchema: PrivateKeyUsagSchema = z.enum(
  PRIVATE_KEY_USAGE,
);
...
export const keyUsageSchema: KeyUsageSchema = z.enum(KEY_USAGE);
 ...
 */
type JwkBaseSchema = z.ZodObject<{
  kty: z.ZodString;
  ...
  >;
  key_ops: z.ZodOptional<z.ZodArray<KeyUsageSchema>>;
...
const jwkBaseSchema: JwkBaseSchema = z.object({
  kty: z.string().min(1),
...
export function isPrivateJwk<J extends object>(jwk: J): jwk is
  | (J & {
