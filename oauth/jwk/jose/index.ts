...
import { either, type RequiredKey } from "./util.ts";
...
export class JoseKey<J extends Jwk = Jwk> extends Key<J> {
  ...
  static async fromImportable(
    ...
    if (typeof input === "object") {
      ...
      if ("kty" in input || "alg" in input) {
        return this.fromJWK(input as Record<string, unknown>, kid);
      }
      ...
      throw new JwkError("Invalid input: expected JWK or CryptoKey");
    }
  ...
  static async fromJWK(
    ...
    const kid = await either(jwk.kid, inputKid);
