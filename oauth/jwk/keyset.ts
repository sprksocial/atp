...
import type { PrivateKeyUsage } from "./jwk.ts";
import type { JwksPub } from "./jwks.ts";
import { unsafeDecodeJwt } from "./jwt-decode.ts";
import type { VerifyOptions, VerifyResult } from "./jwt-verify.ts";
import type { JwtHeader, JwtPayload, SignedJwt } from "./jwt.ts";
import type { ActivityCheckOptions, Key, KeyMatchOptions } from "./key.ts";
import {
  cachedGetter,
  isDefined,
  matchesAny,
...
export class Keyset<K extends Key = Key> implements Iterable<K> {
  ...
  get(options: FindKeyOptions): K {
    ...
    throw new JwkError(
      `Key not found ${
        options.kid ?? options.alg ?? options.usage ?? "<unknown>"
      }`,
      ERR_JWK_NOT_FOUND,
    );
  }
