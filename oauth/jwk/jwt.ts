import { z } from "zod";
import { jwkPubSchema } from "./jwk.ts";
import {
  jwtCharsRefinement,
  segmentedStringRefinementFactory,
} from "./util.ts";

export const signedJwtSchema: z.ZodString = z
  .string()
...
export const unsignedJwtSchema: z.ZodString = z
  .string()
  .superRefine(jwtCharsRefinement)
...
type JwtHeaderSchema = z.ZodObject<{
  alg: z.ZodString;
 ...
 */
export const jwtHeaderSchema: JwtHeaderSchema = z
  .object({
 ...
 */
export const htuSchema: z.ZodString = z.string().superRefine((value, ctx) => {
  try {
