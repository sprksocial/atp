...
export type RequiredKey<T, K extends keyof T = never> = Simplify<
  & T
  & {
    [L in K]-?: unknown extends T[L] ? NonNullable<unknown> | null
...
export const preferredOrderCmp = <T>(order: readonly T[]) => (a: T, b: T) => {
...
export const segmentedStringRefinementFactory = <C extends number>(
  count: C,
  minPartLength = 2,
): (data: string, ctx: RefinementCtx) => void => {
  ...
  return (data: string, ctx: RefinementCtx): void => {
