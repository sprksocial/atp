...
import { DidResolver, ensureAtpDocument } from "../mod.ts";

Deno.test("throws on bad DID document", () => {
...
Deno.test("parses legacy DID format, extracts atpData", () => {
  const did = "did:plc:yk4dd2qkboz2yv6tpubpc6co";
  const docJson = `{
  "@context": [
  ...
  assertEquals(atpData.did, did);
  assertEquals(atpData.handle, "dholms.xyz");
  assertEquals(atpData.pds, "https://bsky.social");
  assertEquals(
    atpData.signingKey,
...
Deno.test("parses newer Multikey DID format, extracts atpData", () => {
  const did = "did:plc:yk4dd2qkboz2yv6tpubpc6co";
  const docJson = `{
  "@context": [
    "https://www.w3.org/ns/did/v1",
