import { ensureValidDid, ensureValidDidRegex } from "./did.ts";
import { ensureValidHandle, ensureValidHandleRegex } from "./handle.ts";
import { isValidNsid } from "./nsid.ts";
import { ensureValidRecordKey } from "./recordkey.ts";
...
export const ensureValidAtUri = (uri: string): void => {
  ...
  if (parts.length >= 5) {
    ...
    try {
      ensureValidRecordKey(parts[4]);
    } catch {
      throw new Error("ATURI record key must be a valid record key");
    }
  }
...
export const ensureValidAtUriRegex = (uri: string): void => {
  ...
  if (groups.rkey) {
    try {
      ensureValidRecordKey(groups.rkey);
    } catch {
      throw new Error("ATURI record key must be a valid record key");
    }
  }
